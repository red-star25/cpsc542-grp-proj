...
(async function loginFormTest() {
  ...
  try {
    ...
    const emptyEmailError = await driver.findElement(By.css(".MuiFormHelperText-root"));
    const emptyEmailErrorMessage = await emptyEmailError.getText();
    assert.strictEqual(emptyEmailErrorMessage, "Email is required");
