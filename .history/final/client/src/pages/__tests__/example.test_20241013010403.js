...
(async function loginFormTest() {
  ...
  try {
    ...
    // Tooltip should be present when email is empty or invalid
    const tooltip = await driver.findElement(By.css(".MuiTooltip-tooltip"));
    const tooltipText = await tooltip.getText();
    assert.strictEqual(tooltipText, "Please enter a valid email");
