...
describe('Launches Component', () => {
    ...
    it('should load more launches', async () => {
        ...
        const initialLaunchTiles = await driver.findElements(By.className('launch-tile'));

        const loadMoreButton = await driver.findElement(By.className('load-more-button'));
        await loadMoreButton.click();

        await driver.wait(until.stalenessOf(loadMoreButton));

        const updatedLaunchTiles = await driver.findElements(By.className('launch-tile'));

        expect(updatedLaunchTiles.length).toBeGreaterThan(initialLaunchTiles.length);
    });
});