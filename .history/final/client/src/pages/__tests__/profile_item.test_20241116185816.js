 ...
 const profileLink = await driver.wait(
  ...
  try {
    ...
    const starlinkText = await driver.wait(
        until.elementLocated(By.xpath("//p[text()='You haven't booked any trips']")),
        TIMEOUT
      );
...
async function cancelTrip(driver){
  ...
  await starlinkElement.click();
   ...
   // find button Cancel Trip and click
  ...
  await profile.click();
   ...
   const noTripText = await driver.wait(
    until.elementLocated(By.xpath("//p[text()='You haven't booked any trips']")),
    TIMEOUT
  );
  ...
  try {
   const textContent = await noTripText.getText();
    assert.strictEqual(textContent, "You haven't booked any trips");
    console.log("Test Passed: item removed from profile.");
  } catch (error) {
    throw new Error("Test Failed Cancel Trip: Title not found");
}
